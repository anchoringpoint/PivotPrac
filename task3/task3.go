...
func directionlite_driving() {
	...
	if tatics == 0 {
		...
		fmt.Println("时间:", fmt.Sprintf("%d小时%d分钟%d秒\n", hours, minutes, seconds))

		fmt.Println("路线信息：")
		for step := range viper.Get(fmt.Sprintf("result.routes.%d.steps", result_route)).([]interface{}) {
			fmt.Println(viper.Get(fmt.Sprintf("result.routes.%d.steps.%d.instruction", result_route, step)))
		}
	} else {
		...
		out_duration = viper.Get(fmt.Sprintf("result.routes.%d.duration", route)).(float64) - out_duration + sum_duration
		fmt.Println(tatics_string[tatics-1], "的路线：")
		hours, minutes, seconds := secondsToHMS(int(out_duration))
		fmt.Println("时间:", fmt.Sprintf("%d小时%d分钟%d秒\n", hours, minutes, seconds))

		fmt.Println("路线信息：")
		for step := range viper.Get(fmt.Sprintf("result.routes.%d.steps", route)).([]interface{}) {
			fmt.Println(viper.Get(fmt.Sprintf("result.routes.%d.steps.%d.instruction", route, step)))
		}
...
func directionlite_transit() {
...

	{
		...
		fmt.Println("价格:", fmt.Sprintf("%.1f元\n", result_price))
	}
